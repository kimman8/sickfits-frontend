...
var utils = require('./utils-d6a80347.cjs.prod.js');
...
async function pushPrismaSchemaToDatabase(dbUrl, schema, schemaPath, shouldDropDatabase = false) {
  ...
  let migration = await withMigrate(dbUrl, schemaPath, async migrate => {
    if (shouldDropDatabase) {
      ...
      let migration = await runMigrateWithDbUrl(dbUrl, () => migrate.engine.schemaPush({
        force: true,
        schema
      }));
    ...
    if (migration.unexecutable.length) {
      ...
      if (!(await utils.confirmPrompt(`Do you want to continue? ${chalk__default['default'].red('All data will be lost')}.`))) {
        console.log('Reset cancelled');
        throw new utils.ExitError(0);
      }

      await runMigrateWithDbUrl(dbUrl, () => migrate.reset());
      return runMigrateWithDbUrl(dbUrl, () => migrate.engine.schemaPush({
        force: false,
        schema
      }));
    }
    ...
    if (migration.warnings.length) {
      ...
      if (!(await utils.confirmPrompt(`Do you want to continue? ${chalk__default['default'].red('Some data will be lost')}.`))) {
...
async function devMigrations(dbUrl, prismaSchema, schemaPath, shouldDropDatabase) {
...
We need to reset the ${credentials.type} database "${credentials.database}" at ${getDbLocation(credentials)}.`);
    ...
    if (evaluateDataLossResult.migrationSteps) {
      ...
      let shouldApplyMigration = migrationCanBeApplied && (await utils.confirmPrompt('Would you like to apply this migration?'));
