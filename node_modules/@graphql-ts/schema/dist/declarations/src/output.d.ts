...
object & Iterable<InferValueFromOutputType<Value>> : Type extends ObjectType<infer RootVal, any> ? RootVal : Type extends UnionType<infer RootVal, any> ? RootVal : Type extends InterfaceType<infer RootVal, any, any> ? RootVal : never;
...
export declare type ObjectType<RootVal, Context> = {
    kind: "object";
...
export declare type FieldResolver<RootVal, Args extends Record<string, Arg<InputType>>, TType extends OutputType<Context>, Context> = (rootVal: RootVal, args: InferValueFromArgs<Args>, context: Context, info: GraphQLResolveInfo) => InferValueFromOutputType<TType>;
/**
 * A GraphQL output field for an {@link ObjectType} which should be created using
 * {@link field `graphql.field`}.
 */
export declare type Field<RootVal, Args extends Record<string, Arg<InputType>>, TType extends OutputType<Context>, Key extends string, Context> = {
    args?: Args;
    type: TType;
    __key: Key;
    ...
    description?: string;
    extensions?: Readonly<GraphQLFieldExtensions<RootVal, Context, InferValueFromArgs<Args>>>;
};
...
declare type FieldFuncResolve<RootVal, Args extends {
    ...
    Key
] extends [keyof RootVal] ? RootVal[Key] extends InferValueFromOutputType<Type> | ((args: InferValueFromArgs<Args>, context: Context, info: GraphQLResolveInfo) => InferValueFromOutputType<Type>) ? {
    resolve?: FieldResolver<RootVal, SomeTypeThatIsntARecordOfArgs extends Args ? {} : Args, Type, Context>;
} : {
    resolve: FieldResolver<RootVal, SomeTypeThatIsntARecordOfArgs extends Args ? {} : Args, Type, Context>;
} : {
    resolve: FieldResolver<RootVal, SomeTypeThatIsntARecordOfArgs extends Args ? {} : Args, Type, Context>;
...
export declare type FieldFunc<Context> = <RootVal, Type extends OutputType<Context>, Key extends string, Args extends {
    [Key in keyof Args]: Arg<InputType>;
} = {}>(field: FieldFuncArgs<RootVal, Args, Type, Key, Context>) => Field<RootVal, Args, Type, Key, Context>;
...
declare type InterfaceFieldToOutputField<RootVal, Context, TField extends InterfaceField<any, any, Context>, Key extends string> = TField extends InterfaceField<infer Args, infer OutputType, Context> ? Field<RootVal, Args, OutputType, Key, Context> : never;
declare type InterfaceFieldsToOutputFields<RootVal, Context, Fields extends {
    [Key in keyof Fields]: InterfaceField<any, any, Context>;
}> = {
    [Key in keyof Fields]: InterfaceFieldToOutputField<RootVal, Context, Fields[Key], Extract<Key, string>>;
};
declare type UnionToIntersection<T> = (T extends any ? (x: T) => any : never) extends (x: infer R) => any ? R : never;
export declare type InterfacesToOutputFields<RootVal, Context, Interfaces extends readonly InterfaceType<RootVal, any, Context>[]> = UnionToIntersection<InterfaceFieldsToOutputFields<RootVal, Context, InterfaceToInterfaceFields<Interfaces[number]>>>;
/**
 * Creates a GraphQL object type.
 *
 * See the docs of {@link object `graphql.object`} for more details.
 */
export declare type ObjectTypeFunc<Context> = <RootVal>(youOnlyNeedToPassATypeParameterToThisFunctionYouPassTheActualRuntimeArgsOnTheResultOfThisFunction?: {
    youOnlyNeedToPassATypeParameterToThisFunctionYouPassTheActualRuntimeArgsOnTheResultOfThisFunction: true;
}) => <Fields extends {
    [Key in keyof Fields]: Field<RootVal, any, any, Extract<Key, string>, Context>;
} & InterfacesToOutputFields<RootVal, Context, Interfaces>, Interfaces extends readonly InterfaceType<RootVal, any, Context>[] = []>(config: {
    name: string;
...
export declare type UnionType<RootVal, Context> = {
    kind: "union";
    __rootVal: RootVal;
    __context: (context: Context) => void;
...
export declare type UnionTypeFunc<Context> = <TObjectType extends ObjectType<any, Context>>(config: {
    ...
    description?: string;
    types: TObjectType[];
    resolveType?: (type: TObjectType["__rootVal"], context: Parameters<TObjectType["__context"]>[0], info: GraphQLResolveInfo, abstractType: GraphQLUnionType) => string;
}) => UnionType<TObjectType["__rootVal"], Parameters<TObjectType["__context"]>[0]>;
export declare type FieldsFunc<Context> = <RootVal>(youOnlyNeedToPassATypeParameterToThisFunctionYouPassTheActualRuntimeArgsOnTheResultOfThisFunction?: {
    youOnlyNeedToPassATypeParameterToThisFunctionYouPassTheActualRuntimeArgsOnTheResultOfThisFunction: true;
}) => <Fields extends {
    [Key in keyof Fields]: Field<RootVal, any, any, Extract<Key, string>, Context>;
}>(fields: Fields) => Fields;
declare type InterfaceFieldFuncArgs<RootVal, Args extends {
    [Key in keyof Args]: Arg<InputType>;
...
export declare type InterfaceFieldFunc<Context> = <RootVal, Type extends OutputType<Context>, Args extends {
    [Key in keyof Args]: Arg<InputType>;
} = {}>(field: InterfaceFieldFuncArgs<RootVal, Args, Type, Context>) => InterfaceField<Args, Type, Context>;
export declare type InterfaceType<RootVal, Fields extends Record<string, InterfaceField<any, OutputType<Context>, Context>>, Context> = {
    kind: "interface";
    __rootVal: (rootVal: RootVal) => void;
    __context: (context: Context) => void;
...
export declare type InterfaceTypeFunc<Context> = <RootVal>(youOnlyNeedToPassATypeParameterToThisFunctionYouPassTheActualRuntimeArgsOnTheResultOfThisFunction?: {
    youOnlyNeedToPassATypeParameterToThisFunctionYouPassTheActualRuntimeArgsOnTheResultOfThisFunction: true;
}) => <Fields extends {
    [Key in keyof Fields]: InterfaceField<any, OutputType<Context>, Context>;
} & UnionToIntersection<InterfaceToInterfaceFields<Interfaces[number]>>, Interfaces extends readonly InterfaceType<RootVal, any, Context>[] = []>(config: {
    name: string;
    description?: string;
    deprecationReason?: string;
    interfaces?: [...Interfaces];
