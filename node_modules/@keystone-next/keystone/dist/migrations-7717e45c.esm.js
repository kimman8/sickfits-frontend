...
import { c as confirmPrompt, E as ExitError, t as textPrompt } from './utils-b84e12de.esm.js';
...
async function pushPrismaSchemaToDatabase(dbUrl, schema, schemaPath, shouldDropDatabase = false) {
  ...
  let migration = await withMigrate(dbUrl, schemaPath, async migrate => {
    ...
    if (migration.unexecutable.length) {
      ...
      if (!(await confirmPrompt(`Do you want to continue? ${chalk.red('All data will be lost')}.`))) {
    ...
    if (migration.warnings.length) {
      ...
      if (!(await confirmPrompt(`Do you want to continue? ${chalk.red('Some data will be lost')}.`))) {
