...
import { g as getGqlNames } from './core-3f0f7b15.esm.js';
import { g as graphqlBoundToKeystoneContext, J as JSON$1, Q as QueryMode } from './sqlite-943cbaa7.esm.js';
import _objectWithoutProperties from '@babel/runtime/helpers/esm/objectWithoutProperties';
import { GraphQLString, executeSync, parse, GraphQLUnionType, GraphQLScalarType, GraphQLNonNull, GraphQLSchema, GraphQLObjectType, assertScalarType, assertEnumType, execute, astFromValue, GraphQLList, validate, print, graphql as graphql$1 } from 'graphql';
...
import { s as staticAdminMetaQuery } from './admin-meta-graphql-c4fe434c.esm.js';
import { bindGraphQLSchemaAPIToContext } from '@graphql-ts/schema';
import { sessionSchema } from '../session/dist/keystone.esm.js';
...
import { arg, nonNull, list, inputObject, Int, ID, Boolean } from '@graphql-ts/schema/api-without-context';
import { v4 } from 'uuid';
import fromBuffer from 'image-type';
import imageSize from 'image-size';
import { parseImageRef } from '../fields/types/image/utils/dist/keystone.esm.js';
...
import { parseFileRef } from '../fields/types/file/utils/dist/keystone.esm.js';
...
function createAdminMeta(config, initialisedLists) {
  ...
  for (const [key, list] of Object.entries(initialisedLists)) {
    ...
    if (((_config$lists$key$ui2 = config.lists[key].ui) === null || _config$lists$key$ui2 === void 0 ? void 0 : _config$lists$key$ui2.searchFields) === undefined) {
      ...
      if ((potentialFilterField === null || potentialFilterField === void 0 ? void 0 : potentialFilterField.type) === GraphQLString) {
    ...
    for (const [fieldKey, field] of Object.entries(list.fields)) {
      ...
      if (searchFields.has(fieldKey)) {
        ...
        if (((_whereInputFields$ = whereInputFields[`${fieldKey}_contains_i`]) === null || _whereInputFields$ === void 0 ? void 0 : _whereInputFields$.type) === GraphQLString) {
          search = 'insensitive';
        } else if (((_whereInputFields$2 = whereInputFields[`${fieldKey}_contains`]) === null || _whereInputFields$2 === void 0 ? void 0 : _whereInputFields$2.type) === GraphQLString) {
          search = 'default';
        } else {
...
function getAdminMetaSchema({
  ...
  const jsonScalar = jsonScalarType ? graphql.scalar(assertScalarType(jsonScalarType)) : JSON$1;
  const queryModeEnumType = graphQLSchema.getType('QueryMode');
  const queryModeEnum = queryModeEnumType ? _objectSpread(_objectSpread({}, QueryMode), {}, {
    graphQLType: assertEnumType(graphQLSchema.getType('QueryMode'))
  }) : QueryMode;
  ...
  return new GraphQLSchema(_objectSpread(_objectSpread({}, schemaConfig), {}, {
    ...
    query: new GraphQLObjectType(_objectSpread(_objectSpread({}, queryTypeConfig), {}, {
...
function getNamesFromList(listKey, {
  ...
  const _singular =
  /*singular ||*/
  pluralize.singular(_label);

  const _plural =
  /*plural ||*/
  pluralize.plural(_label);
...
const accessDeniedError = () => new ApolloError('You do not have access to this resource');
...
class NestedMutationState {
  ...
  async create(data, list) {
    ...
    const writeLimit = pLimit(1); // Check operation permission to pass into single operation
...
async function createMany(createInputs, list, context, provider) {
  const writeLimit = pLimit(provider === 'sqlite' ? 1 : Infinity); // Check operation permission to pass into single operation
...
function getMutationsForList(list$1, provider) {
  ...
  const createMany$1 = field({
    ...
    async resolve(_rootVal, args, context) {
      return promisesButSettledWhenAllSettledAndInOrder(await createMany(args, list$1, context, provider));
    }
  ...
  const updateMany$1 = field({
    ...
    async resolve(_rootVal, args, context) {
      return promisesButSettledWhenAllSettledAndInOrder(await updateMany(args, list$1, context, provider));
    }
  ...
  const deleteMany$1 = field({
    ...
    async resolve(rootVal, {
      ...
      return promisesButSettledWhenAllSettledAndInOrder(await deleteMany(where, list$1, context, provider));
    }
  ...
  return {
    ...
    getKeystone: PrismaClient => {
      ...
      const createContext = makeCreateContext({
        ...
        config,
        prismaClient,
        gqlNamesByList: Object.fromEntries(Object.entries(lists).map(([listKey, list]) => [listKey, getGqlNames(list)])),
        lists
      });
      return {
        async connect() {
...
export { getDBProvider as a, createSystem as c, getDBFieldKeyForFieldOnMultiField as g, initialiseLists as i, serializePathForImport as s, writeAdminFiles as w };
